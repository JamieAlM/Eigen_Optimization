...
int main(){	
   ...
   cross_product(u, B, uxB);
	...
	/*std::ofstream file1("ZZCpp.txt");
    if (file1.is_open()) {//zxy gives correct xx, but not yy
        			for (int i = 0; i < nx; ++i) { 
            			for (int j = 0; j < ny; ++j) {
							for (int k = 0; k < nz; ++k) {
                        		file1 << ZZ[k + nz * (j + ny * i)] << '\n'; 
                    }
                }
        }
    }*/
	...
	/*for (int i = 0; i < (nx * ny * nz); ++i){
		Theta[i] = atan2(YY[i]  -y0,  XX[i]  -x0)  - (M_PI/c); 
		Phiz[i] = atan2(ZZ[i]  -z0,  YY[i]  -y0)  - (M_PI/c);
        P0[i] = (pow(XX[i]-x0,2)/pow(rx0,2) + pow(YY[i]-y0,2)/pow(ry0,2) + pow(ZZ[i]-z0,2)/pow(rz0,2) );               
        nep[i] =( d + a * exp(pow(-1. * P0[i],p)) + ((rand() % 101)/100. * (Ampl_2 - Ampl_1) + Ampl_1) ) * n0  ;
        nebg[i] =0.;
        neTot[i] = nep[i]+ nebg[i];
		Tip[i] = 0.; 
		Tep[i] = 0.; 
		Tibg[i] = 1000.;
		Tebg[i] = 1000.;
        TiTot[i] = Tip[i] + Tibg[i];
        TeTot[i] = Tep[i] + Tebg[i];
        phibg[i] = 0.; 
        phip[i] = 0.; 
        phiTot[i] = phibg[i] + phip[i] ; 
        piTot[i] = neTot[i] * TiTot[i] * kb;
        peTot[i] = neTot[i] * TeTot[i] * kb;
        pibg[i] = nebg[i] * Tibg[i] *kb;
        pebg[i] = nebg[i] * Tebg[i] *kb;
        pip[i] = piTot[i] - pibg[i] *kb;
        pep[i] = peTot[i] - pebg[i] *kb;
	...
	out1 = (double*) fftw_malloc((nx*ny*nz)*sizeof(double));
	c2r3D(dndxK,out1);
			std::ofstream file1("dndxCpp.txt");
    		if (file1.is_open())
    		{
				 for (int i = 0; i < nx; ++i) {
					for (int j = 0; j < ny; ++j) {
							for (int k = 0; k < nz; ++k) {
                        		file1 << out1[k + nz * (j + ny * i)] << '\n'; 
                    		}
                		}
        			}
    		}*/
	...
	/*double *out2; //pe1
	out2 = (double*) fftw_malloc((nx*ny*nz)*sizeof(double));
	c2r3D(dndyK,out2);

	std::ofstream file1("dndyCpp.txt"); //this is correct dndycpp but rotated?
    if (file1.is_open())
    {
		for (int i = 0; i < nx; ++i) { 
			for (int j = 0; j < ny; ++j) {
						for (int k = 0; k < nz; ++k) {
                        	file1 << out2[k + nz * (j + ny * i)] << '\n'; 
                    }
                }
        }
    }*/
	...
	std::ofstream file1("vdmex1Cpp.txt");
    if (file1.is_open())
    {
        		for (int i = 0; i < nx; ++i) { 
					for (int j = 0; j < ny; ++j) {
						for (int k = 0; k < nz; ++k) {
                        	file1 << TeTot[k + nz * (j + ny * i)] << '\n'; 
                    }
                }
        }
    }*/
	...
	for (int iter = 0; iter < iter_max; iter++){ //for (int iter = 0; iter < iter_max; iter++){
		...
		/*std::ofstream file1("vdmiyCpp.txt");
    	if (file1.is_open())
    	{
        		for (int i = 0; i < nx; ++i) { 
					for (int j = 0; j < ny; ++j) {
						for (int k = 0; k < nz; ++k) {
                        	file1 << vdmiY[k + nz * (j + ny * i)] << '\n'; 
                    }
                }
        }
    	}*/
...
void Convolve3D( double fk[][ncomp], double gk[][ncomp], double fgk[][ncomp]){
    ...
    for i = 1:2
        ind_pad{i} = [1:N(i)/2 M(i)-N(i)/2+1:M(i)]; //Matlab [2 0 -1] is a row vector
    end
...
void CollFreqk_inertia3D(double nek[][ncomp], double Tik[][ncomp], double Tek[][ncomp], double kb, double eps0, double mi, double me, double ri, double rn, double nn, double Oci, double Oce, double e,double nuink[][ncomp], double nuiek[][ncomp] , double nuiik[][ncomp] , double nuenk[][ncomp] ,double nueek[][ncomp] ,double nueik[][ncomp]  , double isigPk[][ncomp] , double invnk[][ncomp] , double hallIk[][ncomp] ,double hallEk[][ncomp] ){
	...
	/*for(int i = 0; i< (nx*ny*nz); i++){
		Vthi = sqrt( 2. * kb * Ti[i] / mi);
		Vthe = sqrt( 2. * kb * Te[i] / me);
		nuin[i] =  nn * Vthi* M_PI * (ri+rn) * (ri+rn);
		nuen[i] = nn * Vthe * M_PI * rn * rn;
		// Calculate "inverse Pedersen conductivity"
		isigP[i] = 1.0 / (e * ( nuin[i]/ Oci + nuen[i]/ Oce) );
		// Calculate Debye length
		lambdaD = sqrt( eps0 * kb * Te[i]/ (ne[i] * e * e) );					
		// Calculate plasma parameter
		Lambda = 12.0 * M_PI * ne[i] * lambdaD * lambdaD * lambdaD;
		// Calculate electron-electron collision frequency
		nuee[i] = ne[i]* e * e *e *e * log(Lambda/3.0) / ( 2. * M_PI * eps0 * eps0 * me * me * Vthe * Vthe * Vthe);
		// Calculate ion-ion collision frequency
		nuii[i]= nuee[i] * sqrt(me/mi);
		// Calculate ion-electron collision frequency
		nuie[i] = nuee[i] * 0.5 * me / mi;
		// Calculate electron-ion collision frequency
		nuei[i] = nuee[i];
		// Calculate the inverse of the density
		// inverse of ne in Fourier space (which is needed for several terms in the temperature equation )	
