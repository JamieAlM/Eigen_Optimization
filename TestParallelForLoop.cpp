...
static const int nx = 128;//64;//256; //128;
static const int ny = 128;//64;//256; //128; 
static const int nz = 128;//64;//256; //128;
...
MyFftwClass r2c;
MyiFftwClass c2r; //declare r2c1 of type MyFftwClass
int main(){	
	...
	double Dart = 50;//0.1;//0.05; //0.1;// OR 0.11e3;     //m^2/s // Set artifical diffusion constants
    ...
    double Lx = 640;//2*M_PI;//640;//128; //2*128; //32;//2*EIGEN_PI;
    double Ly = 640;//2*M_PI;//640;//128; //32;//2*EIGEN_PI;
    double Lz = 640;//2*M_PI;//640;//128; //32;//4;//32;//2*EIGEN_PI;
 ...
 //for (int nThreads =1; nThreads <= 16; nThreads++){
	...
	r2c.r2cexecute(neTot, neK);
	r2c.r2cexecute(TiTot, TiK);
	r2c.r2cexecute(TeTot, TeK);
	r2c.r2cexecute(phiTot, phiK);
	r2c.r2cexecute(piTot, piK);
	r2c.r2cexecute(peTot, peK);

	
	r2c.r2cexecute(nebg, nebgK);
	r2c.r2cexecute(nep, nepK);
	r2c.r2cexecute(Tebg, TebgK);
	r2c.r2cexecute(Tibg, TibgK);

	r2c.r2cexecute(phibg, phibgK);
	r2c.r2cexecute(phip, phipK);
	r2c.r2cexecute(pibg, pibgK);
	r2c.r2cexecute(pip, pipK);
	r2c.r2cexecute(pebg, pebgK);
	r2c.r2cexecute(pep, pepK);
	...
	double *out1; //pe1
	out1 = (double*) fftw_malloc((nx*ny*nz)*sizeof(double));
	...
	c2r.c2rexecute(vexbKx, vexbX);
	c2r.c2rexecute(vexbKy, vexbY);
	c2r.c2rexecute(vexbKz, vexbZ);
	...
	c2r.c2rexecute(vexbKx0,vexbX0);
	c2r.c2rexecute(vexbKy0, vexbY0);
	c2r.c2rexecute(vexbKz0, vexbZ0);
	...
	c2r.c2rexecute(vexbKx1, vexbX1);
	c2r.c2rexecute(vexbKy1, vexbY1);
	c2r.c2rexecute(vexbKz1, vexbZ1);
	...
	c2r.c2rexecute(vdmexK, vdmeX);
	c2r.c2rexecute(vdmeyK, vdmeY);
	c2r.c2rexecute(vdmezK, vdmeZ);
	c2r.c2rexecute(vdmixK, vdmiX);
	...
	//c2r3D(vdmixK, vdmiX);
	//std::ofstream file("vdmixCpp.txt");
    //if (file.is_open())
    //{
    ...
    //    }
    //}
	c2r.c2rexecute(vdmiyK, vdmiY);
	c2r.c2rexecute(vdmizK, vdmiZ);
	...
	c2r.c2rexecute(vdmex1K, vdmeX1);
	c2r.c2rexecute(vdmey1K, vdmeY1);
	c2r.c2rexecute(vdmez1K, vdmeZ1);
	...
	c2r.c2rexecute(vdmix1K, vdmiX1);
	c2r.c2rexecute(vdmiy1K, vdmiY1);
	c2r.c2rexecute(vdmiz1K, vdmiZ1);
	...
	c2r.c2rexecute(neK, neTot);
	c2r.c2rexecute(TeK, TeTot);
	c2r.c2rexecute(TiK, TiTot);
	c2r.c2rexecute(phiK, phiTot);
	...
	//double *out; //pe1
	//out = (double*) fftw_malloc((nx*ny*nz)*sizeof(double));
	...
	for (int iter = 0; iter < 1; iter++){ //for (int iter = 0; iter < iter_max; iter++){
       ...
       time[iter + 1] = time[iter] + dt; 
		for (int i = 0; i < (nx * ny * nzk); ++i){
			for (int l = 0; l < ncomp; l++){
				ne1K_old[i][l] = nepK[i][l] ; // change to perturbed all of them: ne1k
				TeK_old[i][l] = TeK[i][l] ; //Tek and Ti are pertrubed
				TiK_old[i][l] = TiK[i][l] ;
				phi1K_old[i][l] = phipK[i][l] ; 
			}
		}
        ...
        for (int stage = 0; stage < 1; stage++){ //for (int stage = 0; stage < 4; stage++){
			...
			for (int i = 0; i < (nx * ny * nzk); ++i){
				for( int l = 0; l < ncomp; l++){
					neK[i][l] = nebgK[i][l] + nepK[i][l];
					phiK[i][l] = phibgK[i][l] + phipK[i][l];
				}
			}			
			...
			for (int i = 0; i < (nx * ny * nzk); ++i){
				for( int l = 0; l < ncomp; l++){
					pepK[i][l] = peK[i][l] + pebgK[i][l];
					pipK[i][l] = piK[i][l] + pibgK[i][l];
				}
			}			
			...
			for (int i = 0; i < (nx * ny * nzk); ++i){	
				for( int l = 0; l < ncomp; l++){
					veoxK[i][l] = vdmexK[i][l] + vexbKx[i][l];
					veoyK[i][l] = vdmeyK[i][l] + vexbKy[i][l];
					veozK[i][l] = vdmezK[i][l] + vexbKz[i][l];
							
					vioxK[i][l] = vdmixK[i][l] + vexbKx[i][l];
					vioyK[i][l] = vdmiyK[i][l] + vexbKy[i][l];
					viozK[i][l] = vdmizK[i][l] + vexbKz[i][l];
		...
		if ((iter/saveFrequency) - saveNum == 0){
			...
			char timefilename[16] = {0}; // time[iter + 1]
			strcat(timefilename, "time");
			strcat(timefilename, save);
			strcat(timefilename, type);

			FILE *fptr;
			fptr = fopen(timefilename, "w");
			fprintf(fptr, "%+4.16le \n ",time[iter+1]);
  			fclose(fptr);

			std::ofstream file1( file_name_for_iteration(iter) );//("ZZCpp.txt");
			if (file1.is_open()) {//zxy gives correct xx, but not yy
        			for (int i = 0; i < nx; ++i) { 
            			for (int j = 0; j < ny; ++j) {
							for (int k = 0; k < nz; ++k) {
                        		file1 << neTot[k + nz * (j + ny * i)] << '\n'; 
                    		}
                		}
       				 }
    		}
	...
	//works for MATLAB printing ZXY: this returns 5 matrices v(:,:,5) each matrix is 8-by-8 for nx=ny=nz=8, which in MATLAB this is 8 matrices v(:,:,8) each one is 8-by-8 due to symmetry
	/*std::ofstream file1("KXCpp.txt");
    if (file1.is_open())
    {
         for (int k = 0; k < nzk; ++k) {//kx must be nzk to be correct b/c nzk is the third n/2+1 direction
        		for (int i = 0; i < nx; ++i) {
            		for (int j = 0; j < ny; ++j) {
                        	file1 << KXX[k + nzk * (j + ny * i)] << '\n'; 
                    }
	...
	/*std::ofstream file("CppnTe.txt");
    if (file.is_open())
    {
         //kx must be nzk to be correct
        		for (int i = 0; i < nx; ++i) {
            		for (int j = 0; j < ny; ++j) {
						for (int k = 0; k < nz; ++k) {
                        	file << TeTot[k + nz * (j + ny * i)] << '\n'; 
                    }
                }
        }
    }*/
    ...
    CollFreqk_inertia3D( neK, TiK, TeK, kb, eps0, mi, me, ri, rn, nn, Oci, Oce, e, nuinK, nuieK , nuiiK , nuenK ,nueeK ,nueiK , isigPK , invnK , hallIK , hallEK );
	//c2r3D(hallEK,out1);
	//std::ofstream file("Cpphalle.txt");
    //if (file.is_open())
    //{
    //     //kx must be nzk to be correct
    //   		for (int i = 0; i < nx; ++i) {
...
void scalArr2DMult(double arr[], double scal, double arrOut[]){
	...
	for(int i = 0; i < nx; i++){
		for( int j = 0; j < ny; j++){
			for( int k = 0; k < nz; k++){
				arrOut[k + nz * (j + ny * i)] = arr[k + nz * (j + ny * i)] * scal;			
			}
		}
	}
}
//new FFTs with class definition
std::mutex g_my_fftw_mutex1; // <-- must lock before using any fftw_*() function, except for fftw_execute*()
MyiFftwClass::MyiFftwClass(void)
{
    // serialize the initialization!
    std::lock_guard<std::mutex> lock(g_my_fftw_mutex1);

	// allocate separate buffers for each instance
    m_buffer_in1 = fftw_alloc_complex((nx*ny*nzk));
    m_buffer_out1 = fftw_alloc_real((nx*ny*nz)); 
	...
	//int N_Threads = omp_get_max_threads();
	//fftw_plan_with_nthreads(N_Threads); //before creating a plan that you want to parallelize, you should call.
	//You can call fftw_plan_with_nthreads, create some plans, call fftw_plan_with_nthreads again with a different argument, 
	//and create some more plans for a new number of threads. Plans already created before a call to fftw_plan_with_nthreads are unaffected.
    ...
    if (!s_plan1)
    {
		...
		s_plan1 = fftw_plan_dft_c2r_3d(nx, ny,nz, m_buffer_in1, m_buffer_out1, FFTW_PATIENT);
...
void MyiFftwClass::c2rexecute(double cArr[][ncomp],double rArr[]) //void MyFftwClass::execute(double *const in, fftw_complex *const out)
{
	...
	memcpy(m_buffer_in1,cArr, sizeof(fftw_complex) * (nx*ny*nzk));
    fftw_execute_dft_c2r(s_plan1, m_buffer_in1, m_buffer_out1); //instead of fftw_excute(plan)
	memcpy(rArr, m_buffer_out1,sizeof(double) * (nx*ny*nz));
	scalArr2DMult(rArr, 1.0 / (nx*ny*nz), rArr); //renormalize 
}
...
MyFftwClass::MyFftwClass(void)
{
	...
	// allocate separate buffers for each instance
    m_buffer_in = fftw_alloc_real((nx*ny*nz)); 
    m_buffer_out = fftw_alloc_complex((nx*ny*nzk));
   ...
   if (!s_plan)
    {
		...
		s_plan = fftw_plan_dft_r2c_3d(nx, ny,nz, m_buffer_in, m_buffer_out, FFTW_PATIENT);
...
void MyFftwClass::r2cexecute(double rArr[], double cArr[][ncomp]) //void MyFftwClass::execute(double *const in, fftw_complex *const out)
{
    // No serialization is needed here
    memcpy(m_buffer_in, rArr,  sizeof(double) * (nx*ny*nz));
    fftw_execute_dft_r2c(s_plan, m_buffer_in, m_buffer_out); //instead of fftw_excute(plan)
    memcpy(cArr, m_buffer_out, sizeof(fftw_complex) * (nx*ny*nzk));
}
...
double makeSpatialMesh3D(double dx, double dy, double dz,double xarr[], double yarr[],double zarr[]){
	for(int i = 0; i< nx; i++){
		for(int j = 0; j< ny; j++){ 
            for(int l =0; l<nz; l++){
                /*xarr[l + nz * (j + ny * i)] = i*dx; //j*dx;//this index returns same indexing as Eigen
                yarr[l + nz * (j + ny * i)] = l*dy; //l*dy;
                zarr[l + nz * (j + ny * i)] = j*dz;//i*dz;//j*dz;*/

				xarr[l + nz * (j + ny * i)] = i*dx; //j*dx;//this index returns same indexing as Eigen
                yarr[l + nz * (j + ny * i)] = j*dy; //l*dy;
                zarr[l + nz * (j + ny * i)] = l*dz;//i*dz;//j*dz;
            }		
...
//void FourierMesh3D(double Lx,double Ly,double Lz, double kx[], double ky[], double kz[], double KX[], double KY[],double KZ[], double ksqu[], double ninvksqu[], int FourierMeshType){
		...
		for(int j = 0; j < ny; j++){	
			for (int k = 0; k < nzk; ++k) {			
				...
				if( i >= nx/2){ // i >= nx/2 --> from -128 to -1
					KX[k + nzk * (j + ny * i)] =  2.* M_PI * (-i + 2.*kx_counter) / Lx; //2.* M_PI * (-i + 2.*k_counter) / Lx;
				}
	...
	for(int j = 0; j < ny ; j++){
		for(int i = 0; i < nx; i++){	
			for (int k = 0; k < nzk; ++k) {			
				if (j < ny/2){
					KY[k + nzk * (j + ny * i)] =2*M_PI*j/Ly; //2*M_PI*j/Ly;	

				}
				if (j >= ny/2){
					KY[k + nzk * (j + ny * i)] =  2.* M_PI * (-j + 2.*ky_counter) / Ly;		
	...
	// Make kz. Because we are using special FFTs for purely real valued functions, the last dimension (z in this case) only need n/2 + 1 points due to symmetry about the imaginary axis.
	#pragma omp parallel for schedule(static) collapse(3)
	for(int i = 0; i < nx; i++){
		for(int j = 0; j < ny; j++){	
			for (int k = 0; k < nzk; ++k) {		
				KZ[k + nzk * (j + ny * i)] =2*M_PI*k/Lz;
			}						
		}	
	}
	/*int i=1,j =0, k=0;
	for(int k = 0; k < nzk; k++){
		std::cout << KZ[k + nzk * (j + ny * i)] << endl; //KZ(1,1,:)
	}*/
	...
	/*int i=1,j =0, k=0;
	for(int i = 0; i < nx; i++){
		std::cout << KX[k + nzk * (j + ny * i)] << endl; //KZ(1,1,:)
	}*/
...
void c2r3D(double cArr[][ncomp], double rArr[]){
	...
	//fftw_plan backward = fftw_plan_dft_c2r_3d(nx, ny, nz, &cArr[0], &rArr[0], FFTW_ESTIMATE);
...
void Derivk3D(double vark[][ncomp], double K[], double derivative[][ncomp]){
	...
	//auto idx = k + nzk * (j + ny * i); std::swap(Output[idx][REAL], Output[idx][IMAG]); Output[idx][REAL] = -Output[idx][REAL]; would be clearer.
	/*clock_t start_time1 = clock();
	double dummy; 
	unsigned i = 0, j=0, k=0;
	auto idx = k + nzk * (j + ny * i);
	for (int idx = 0; idx < nx; ++idx){
		for (int l = 0; l < ncomp; ++l){
			derivative[idx][l] = vark[idx][l] * K[idx];
			dummy = derivative[k + nzk * (j + ny * i)][REAL];
            derivative[idx][REAL] = -derivative[idx][IMAG];
            derivative[idx][IMAG] = dummy;
			std::cout<< derivative[idx][l]<<endl;
		}
	...
	/*for (int nThreads =1; nThreads <= 16; nThreads++){
			...
			for (int i = 0; i < (nx * ny * nzk); ++i){ 
				derivative[i][0] = -vark[i][1] * K[i]; 
				derivative[i][1] = vark[i][0] * K[i]; }
        		...
        		for (int j = 0; j < ny; ++j) {
            		for (int k = 0; k < nzk; ++k) {
                        for (int l = 0; l < ncomp; ++l){
							...
							//cout << " y: "  << derivative[k + nzk * (j + ny * i)][l]  << "\n"; 
...
void CollFreqk_inertia3D(double nek[][ncomp], double Tik[][ncomp], double Tek[][ncomp], double kb, double eps0, double mi, double me, double ri, double rn, double nn, double Oci, double Oce, double e,double nuink[][ncomp], double nuiek[][ncomp] , double nuiik[][ncomp] , double nuenk[][ncomp] ,double nueek[][ncomp] ,double nueik[][ncomp]  , double isigPk[][ncomp] , double invnk[][ncomp] , double hallIk[][ncomp] ,double hallEk[][ncomp] ){
	...
	c2r.c2rexecute(nek,ne);
	c2r.c2rexecute(Tik,Ti);
	c2r.c2rexecute(Tek,Te);
	...
	r2c.r2cexecute(nuin, nuink);
	r2c.r2cexecute(nuie, nuiek);
	r2c.r2cexecute(nuii, nuiik);
	r2c.r2cexecute(nuen, nuenk);
	r2c.r2cexecute(nuei, nueik);
	r2c.r2cexecute(nuee, nueek);
	r2c.r2cexecute(invn, invnk);
	r2c.r2cexecute(isigP, isigPk);
	r2c.r2cexecute(hallE, hallEk);
	r2c.r2cexecute(hallI, hallIk);
...
double max3Dk(double arr2D[]){ 
	...
	for (int i = 0; i < nx; i++){
		for (int j = 0; j < ny; j++){//for (int j = 0; j < nyk; j++){
			for (int k = 0; k < nzk; k++){
				if (arr2D[k + nzk * (j + ny * i)] > maxVal){ //if (n)--> n isn't initialized here at
					//double *arr2D= new double [j+nyk*i]; //test this isn't initialized

					maxVal = arr2D[k + nzk * (j + ny * i)];
				}
			}
		}
	}	
...
double calc_dt3D(double U[], double vexbx[], double vexby[],double vexbz[] ,double diamagxi[], double diamagyi[], double diamagzi[],double diamagxe[], double diamagye[], double diamagze[],double cfl, double kmax, double maxdt){
	...
	//	int N = 3;
		...
		vMaxArr[3] = max3Dk(diamagxi); //diamagxi is wrong 
		...
		//std::cout<<vMaxArr[3]<<endl; //wrong
		//std::cout<<vMaxArr[4]<<endl; ////wrong
...
void calc_residualn3D(double vexbxk[][ncomp], double vexbyk[][ncomp], double vexbzk[][ncomp], double nink[][ncomp], double residnoutk[][ncomp], double kx[], double ky[], double kz[]){
	...
	/*for(int i = 0; i < (nx*ny*nzk); i++){
		for( int l = 0; l < ncomp; l++){
			residnoutk[i][l] = mult1[i][l] ;//+ mult2[i][l] + mult3[i][l];
		}
	}	*/
	/*double *out; 
	out = (double*) fftw_malloc((nx*ny*nz)*sizeof(double));
	c2r3D(residnoutk,out);
			std::ofstream file1("residualnCpp.txt");
    		if (file1.is_open())
    		{
				for (int i = 0; i < nx; ++i) { 
					for (int j = 0; j < ny; ++j) {
						for (int k = 0; k < nz; ++k) {
                        		file1 << out[k + nz * (j + ny * i)] << '\n'; 
                    	}
                	}
        		}
    		}	*/	
...
void calc_residualt3D(double voxk[][ncomp], double voyk[][ncomp],double vozk[][ncomp], double tempink[][ncomp], double tempoutk[][ncomp], double kx[], double ky[],double kz[]){
	...
	/*for(int i = 0; i < (nx*ny*nzk); i++){	
		for( int l = 0; l < ncomp; l++){
			divvo[i][l] = dvoxk[i][l] + dvoyk[i][l] + dvozk[i][l];
		}
	}	*/		
	...
	/*for(int i = 0; i < (nx*ny*nzk); i++){
		for (int l = 0; l < ncomp; l++){
			tempoutk[i][l] = mult1[i][l] + mult2[i][l] + mult3[i][l] + mult4[i][l] ;
...
double max_absComp3D(double arr3D[][ncomp]){
	...
	double *absArr;
	absArr = (double*) fftw_malloc((nx*ny*nzk)*sizeof(double));
	memset(absArr, 42, (nx*ny*nzk)* sizeof(double)); //test here, if you pass 2D array to func decays to pointer and sizeof doesn't give size of array


	//absComp(arr3D, absArr);
	//void absComp(double arr3D[][ncomp], double arrOut[]){
	#pragma omp parallel for	//faster than schedule
	for (int i = 0; i < nx ; i++){
		for (int j = 0 ; j < ny; j++){
			for (int k = 0; k < nzk; k++){
	...
	/*for(int i = 0; i < (nx*ny*nzk); i++){
		absArr[i] = sqrt(arr3D[i][0]*arr3D[i][0] + arr3D[i][1]*arr3D[i][1]);	
	}*/
...
void calcPotSourcek_inertia3D(double ne0k[][ncomp], double nek[][ncomp],double dndx0k[][ncomp], double dndy0k[][ncomp],double dndz0k[][ncomp], double dndxk [][ncomp], double dndyk [][ncomp],double dndzk [][ncomp], double dphidx0k [][ncomp], double dphidy0k [][ncomp], double dphidz0k [][ncomp], double dphidx1k [][ncomp], double dphidy1k [][ncomp], double dphidz1k [][ncomp],double Pi1k[][ncomp], double Pe1k[][ncomp], double uxB[], double e, double Cm, double hallEk [][ncomp], double hallIk [][ncomp], double vexbkx0[][ncomp], double vexbky0[][ncomp],double vexbkz0[][ncomp], double vexbkx[][ncomp],  double vexbky[][ncomp], double vexbkz[][ncomp],double kx[], double ky[], double kz[], double ksqu[], double potSourcek_inertia[][ncomp]){
	...
	/*for(int i = 0; i < (nx*ny*nzk); i++){
		for (int l = 0; l < ncomp; l++){
			pikTerm[i][l] = -1.*(Cm/e)* hallIk[i][l]; 
			pekTerm[i][l] = (Cm/e)*hallEk[i][l];
		}			
	}*/
	...
	/*for(int i = 0; i < (nx*ny*nzk); i++){
		for (int l = 0; l < ncomp; l++){
			ne1k[i][l] =  nek[i][l] - ne0k[i][l]; 
			dndx1k[i][l] = dndxk[i][l] - dndx0k[i][l];
			dndy1k[i][l] = dndyk[i][l] - dndy0k[i][l];
			dndz1k[i][l] = dndzk[i][l] - dndz0k[i][l];
			dphidxk[i][l] = dphidx0k[i][l] + dphidx1k[i][l];
			dphidyk[i][l] = dphidy0k[i][l] + dphidy1k[i][l];
			dphidzk[i][l] = dphidz0k[i][l] + dphidz1k[i][l];
			vexbkx1[i][l] = vexbkx[i][l] - vexbkx0[i][l];
			vexbky1[i][l] = vexbky[i][l] - vexbky0[i][l];
			vexbkz1[i][l] = vexbkz[i][l] - vexbkz0[i][l];
 ...
 //Take derivatives of dummy variable:
	...
	/*for(int i = 0; i < (nx*ny*nzk); i++){
		for (int l = 0; l < ncomp; l++){
			div_dummyTerm[i][l] = div_dummy_x1[i][l] + div_dummy_y1[i][l] + div_dummy_z1[i][l] + div_dummy_x0[i][l] + div_dummy_y0[i][l] + div_dummy_z0[i][l] ;
			coeff[i][l] = -1.*(Cm)*(hallEk[i][l]+ hallIk[i][l]);
			div_n_nabphi_x[i][l] = div_dummy_dx1[i][l] + div_dummy_dx0[i][l]; 
			div_n_nabphi_y[i][l] = div_dummy_dy1[i][l] + div_dummy_dy0[i][l];
			div_n_nabphi_z[i][l] = div_dummy_dz1[i][l] + div_dummy_dz0[i][l];
		}			
	}*/
 ...
 // convolutions:
	...
	/*for(int i = 0; i < (nx*ny*nzk); i++){
		for (int l = 0; l < ncomp; l++){
			nabnab_dummyTerm_n0v0phi1[i][l] = (nabnab_dummyTerm_n0v0phi1_x[i][l] ) + ( nabnab_dummyTerm_n0v0phi1_xy[i][l] ) + ( nabnab_dummyTerm_n0v0phi1_xz[i][l] );
			nabnab_dummyTerm_n0v0phi1_yy[i][l] = (nabnab_dummyTerm_n0v0phi1_yx[i][l] ) + ( nabnab_dummyTerm_n0v0phi1_y[i][l] ) + ( nabnab_dummyTerm_n0v0phi1_yz[i][l] );
			nabnab_dummyTerm_n0v0phi1_zz[i][l] = (nabnab_dummyTerm_n0v0phi1_zx[i][l] ) + ( nabnab_dummyTerm_n0v0phi1_zy[i][l] ) + ( nabnab_dummyTerm_n0v0phi1_z[i][l] );
			nabnab_dummyTerm_n1vphi[i][l] = (nabnab_dummyTerm_n1vphi_x[i][l] ) + ( nabnab_dummyTerm_n1vphi_xy[i][l] ) + ( nabnab_dummyTerm_n1vphi_xz[i][l] );
			nabnab_dummyTerm_n1vphi_yy[i][l] = (nabnab_dummyTerm_n1vphi_yx[i][l] ) + ( nabnab_dummyTerm_n1vphi_y[i][l] ) + ( nabnab_dummyTerm_n1vphi_yz[i][l] );
			nabnab_dummyTerm_n1vphi_zz[i][l] = (nabnab_dummyTerm_n1vphi_zx[i][l] ) + ( nabnab_dummyTerm_n1vphi_zy[i][l] ) + ( nabnab_dummyTerm_n1vphi_z[i][l] );
			nabnab_dummyTerm_n0v1phi[i][l] = (nabnab_dummyTerm_n0v1phi_x[i][l] ) + ( nabnab_dummyTerm_n0v1phi_xy[i][l] ) + ( nabnab_dummyTerm_n0v1phi_xz[i][l] );
			nabnab_dummyTerm_n0v1phi_yy[i][l] = (nabnab_dummyTerm_n0v1phi_yx[i][l] ) + ( nabnab_dummyTerm_n0v1phi_y[i][l] ) + ( nabnab_dummyTerm_n0v1phi_yz[i][l] );
			nabnab_dummyTerm_n0v1phi_zz[i][l] = (nabnab_dummyTerm_n0v1phi_zx[i][l] ) + ( nabnab_dummyTerm_n0v1phi_zy[i][l] ) + ( nabnab_dummyTerm_n0v1phi_z[i][l] );
 ...
 // Multiply by n0:
   ...
   // try nabnab_dummyTerm_n0v0phi1
		...
		for(int j = 0; j < ny; j++){
			for(int k = 0; k < nzk; k++){
				for(int l = 0; l < ncomp; l++){
					...
					//potSourcek_inertia[k + nzk * (j + ny * i)][l] =    uxBTerm[k + nzk * (j + ny * i)][l];
	...
	/*for(int i = 0; i < (nx*ny*nzk); i++){
		for (int l = 0; l < ncomp; l++){
			nabnab_dummyTerm_n0v0phi1[i][l] = (nabnab_dummyTerm_n0v0phi1_x[i][l] ) + ( nabnab_dummyTerm_n0v0phi1_xy[i][l] ) + ( nabnab_dummyTerm_n0v0phi1_xz[i][l] );
					v_nabnab_phiTerm[i][l] = - (dnabnab_dummyTerm_n0v0phi1_x[i][l] +  dnabnab_dummyTerm_n0v0phi1_y[i][l]  +  dnabnab_dummyTerm_n0v0phi1_z[i][l]
					+ dnabnab_dummyTerm_n1vphi_x[i][l] +  dnabnab_dummyTerm_n1vphi_y[i][l]  +  dnabnab_dummyTerm_n1vphi_z[i][l] 
					+ dnabnab_dummyTerm_n0v1phi_x[i][l] +  dnabnab_dummyTerm_n0v1phi_y[i][l]  +  dnabnab_dummyTerm_n0v1phi_z[i][l] );
				
					potSourcek_inertia[i][l] = v_nabnab_phiTerm[i][l] + div_n_nabphi_x[i][l] + div_n_nabphi_y[i][l] +  pikTerm[i][l] + pekTerm[i][l] + uxBTerm[i][l];
	...
	do{			
		...
		for (int i = 0; i < nx; i++){
			for (int j = 0; j < ny; j++){
				for (int k = 0; k < nzk; k++){
					for (int l = 0; l < ncomp; l++){
					//RHS[k + nzk * (j + ny * i)][l]=  potSourcek[k + nzk * (j + ny * i)][l]  ;
		...
		/*for (int i = 0; i < (nx*ny*nzk); i++){
			for (int l = 0; l < ncomp; l++){
				RHS[i][l]= potSourcek[i][l] - gradNgradPhi_x[i][l] - gradNgradPhi_y[i][l] - gradNgradPhi_z[i][l]  ;
			}
		}*/
